...
# Search run 2016/10/24 - "knee adduction moment"
...
search_topic <- "knee adduction moment"

# check number of results that will be returned from a full search
search_query <- EUtilsSummary(search_topic, retmax = 600)
...
for (i in 1:nrow(pubmed_data)) {
  pubmed_data[i, 7] = pubmed_data_authors[[i]][1, 1]
  pubmed_data[i, 8] = pubmed_data_authors[[i]][1, 2]
  pubmed_data[i, 9] = pubmed_data_authors[[i]][1, 3]
}
...
for (i in 1:nrow(pubmed_data)) {
  ...
  single_paper[[3]] <- paste0("FIRST AUTHOR : ", pubmed_data[i, 7],
                         ", ", pubmed_data[i, 8], ", ", pubmed_data[i, 9])
  ...
  single_paper[[8]] <- paste0("ABSTRACT: ", pubmed_data[i, 3])
  single_paper[[9]] <- ""
...
# Write output
writeLines(unlist(formatted_doc), "abstracts_for_review_V2.txt")
pubmed2 <- pubmed_data
pubmed2 <- pubmed2[,c(7, 4)]
write.table(pubmed2, "pubmed_data.csv", sep = ",")
